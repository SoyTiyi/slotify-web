    ...
    return (
        <nav className="flex items-center justify-between p-4 bg-base text-white relative">
            ...
            <div className="hidden md:flex space-x-2">
                ...
                <button className="bg-primary font-bold text-white px-6 py-2 rounded-xl cursor-pointer hover:bg-primary-hover">Prueba gratis</button>
            </div>

            {isOpen && (
                <div className="absolute top-full left-0 w-full bg-surface flex flex-col items-center py-4 space-y-4 md:hidden shadow-lg z-50 border-t border-gray-700">
                    {links.map((link) => (
                        <NavbarLink key={link.id} href={link.href} label={link.label} />
                    ))}
                    <div className="flex flex-col space-y-2 w-full px-8">
                        ...
                        <button className="bg-primary font-bold text-white px-6 py-2 rounded-xl cursor-pointer w-full hover:bg-primary-hover">Prueba gratis</button>
                    </div>
                </div>
            )}
        </nav>
